...
namespace SdetBootcampDay3.Exercises
{
    ...
    public class Exercises02
    {
        ...
        [TestCase(2, "Ervin Howell", TestName = "User 2 = Ervin Howell")]
        [TestCase(3, "Clementine Bauch", TestName = "User 3 = Clementine Bauch")]
        public async Task CheckUserIsParticularName
            (int idNum, string nameToValidate)
        {
            RestRequest request = new RestRequest($"/users/{idNum}", Method.Get);

            RestResponse response = await client.ExecuteAsync(request);

            JObject responseData = JObject.Parse(response.Content!);

            Assert.That(responseData.SelectToken("name")!.ToString(), Is.EqualTo(nameToValidate));
        }
 /**
        [Test]
